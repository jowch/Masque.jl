...
begin
    function cluster(cx, cy, sx, sy, n, seed)
        ...
        s = UInt32(seed)
        for i in 1:n
            s = 0x41c64e6d * s + UInt32(0x3039)
            u1 = Float64(s >>> 8) / 16777216
            s = 0x41c64e6d * s + UInt32(0x3039)
            u2 = Float64(s >>> 8) / 16777216
            r = sqrt(-2 * log(max(u1, 1.0e-9)))
            θ = 2π * u2
            xs[i] = cx + sx * r * cos(θ)
            ys[i] = cy + sy * r * sin(θ)
        end
    ...
    classes = [
        (name = "Adelie", cx = 38.8, cy = 18.3, sx = 2.2, sy = 1.0, seed = 11, color = "#e07a3d"),
