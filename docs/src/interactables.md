...
| `ViewInteractable(ax; id = :view)` | drag-to-pan (2D) or drag-to-orbit (Axis3); the wheel zooms a 2D view about the cursor; commits nothing; Shift+drag forces view over ROI/threshold | — | none — a camera isn't an analysis value (§12.3); both backends stream a live gesture-channel preview instead |
