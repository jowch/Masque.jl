...
| `PolygonInteractable(ax, rings; holes = nothing, id = :polygons)` | `rings :: Vector{Vector{(x, y)}}` — one exterior ring per polygon | `holes` — one group of hole rings per element, same point type as `rings`. Omitted: every element is solid. A point in a hole is not a hit of that element | [`ElementEvent`](@ref); default field `index` (1-based) |
...
| [`PolygonInteractable`](@ref) | `Poly` (one ring or many); `Band`, `Density` (filled curve); `Contourf` (filled levels; a pointer in a hole misses that polygon and hits the polygon drawn there); `Violin`; `Voronoiplot` (cell polygons) |
