...
- [`ROIInteractable`](@ref) — a draggable + resizable rectangle; drag the interior to move, a
  corner to resize both edges, or the middle of a side to resize just that one edge; commit
  the data-space bounds on mouse-up
...
|---|---|---|---|
| `PointInteractable(ax, points; radius = nothing, radius3d = nothing, id = :points)` | `points :: Vector{(x, y)}` (or `(x,y,z)` for a 3D axis) | `radius` — px highlight radius. Omitted: the drawn radius of the one `Scatter` on `ax` with these positions, otherwise Makie's default `:circle` at the theme `markersize` (≈0.3525×`markersize`). `radius3d` — per-point data-space half-extents on a 3D axis (overrides `radius`) | [`ElementEvent`](@ref); default fields `index` (1-based), `x`, `y`[, `z`] |
...
| `SliceInteractable(ax; series, orientation = :vertical, id = :slice, covers = (), tooltip = nothing)` | samples 1-D series at the cursor. Both crosshair arms are overlay chrome whenever the cursor is `crosshair`; this adds a filled dot per series, in the series colour and ringed in the figure background | `orientation` — `:vertical` samples `y` at data `x` (the default), `:horizontal` samples `x` at data `y`. `covers` — `:polygons` or `:lines` layer ids in the same `masque` call whose highlight this slice replaces. `series` — `(; x, y)` plus optional `id`, `label`, `color`. `tooltip` — `nothing` (auto table), `masque"…"`, or `false` | none — hover only |

The overlay draws both hairlines across the axis viewport (or colorbar bbox) under the pointer whenever the resolved cursor is `crosshair`: an axis or colorbar readout, a grid cell, empty axis interior, or a layer named in `covers`. The hairline is a quieter grey than the selection edge (`#b0b0b0` on a light figure, `#929292` on a dark one), at 80% opacity, with a fringe in the figure's own background so the line stays visible where it crosses a mark. A discrete mark and a legend entry stay `pointer`, and the cross stays off. Threshold, ROI, and view keep their drag cursors. `SliceInteractable` is not a hit target and `masque(fig)` does not add one. One slice per axis.

`SliceInteractable(ax, plot)` and `SliceInteractable(ax, plots)` accept `Lines`, `Stairs`, `Series`, `Band`, and `Density`. Vertices are the points those plots already draw; `Density` and `Band` contribute the band's upper curve. `covers = nothing` (the plot-constructor default) names that plot's auto-extract layer id.
...
`Scatter` on `ax` has those positions in the same order. Pass `radius=` when the lookup would be ambiguous, or
