#!/usr/bin/env node
// Link checker for docs/dev/architecture/*.md.
//
// The architecture doc split (see docs/dev/architecture.md) converted ~58 prose "§N"/"§N.M"
// cross-references into real markdown links so they can be verified mechanically instead of
// trusted by eye. This script walks every relative markdown link under docs/dev/architecture/
// and fails loud on any link whose target file or heading anchor does not actually exist.
//
...
const ARCH_DIR = join(__dirname, "architecture");
...
// [label](target) -- ignore image links, autolinks, and absolute/external URLs.
const LINK_RE = /(?<!!)\[[^\]]*\]\(([^)\s]+)\)/g;
...
for (const file of files) {
  ...
  for (const target of extractLinks(text)) {
    ...
    const targetPath = filePart === "" ? file : resolve(dirname(file), filePart);
...
console.log(`OK: ${checked} relative links checked across ${files.length} files, all resolve.`);
