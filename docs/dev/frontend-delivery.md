...
| **Language/tooling** | **TypeScript + esbuild + eslint + LSP.** | Chosen for whoever maintains this next: types as guardrails, LSP navigation, lint consistency. A typed manifest/`HitLayer` contract catches Julia↔JS drift across the layer kinds (`Kind` in `frontend/src/types.ts`: the closed set of seven data geometry kinds plus `axis`, the `view`/`threshold`/`roi` drag kinds, and `slice`). |
...
| **Bond return** | Typed `InteractionEvent` (or a vector of them for a multi-element selection) via `Bonds.transform_value` (APD ≥0.17.1), in `src/bond.jl`. | Better ergonomics than a raw `Dict`. |
...
| **DPI / sizing** | Render output px ≈ **2 × the display width**; display width = Pluto's **700px** column by default (or `max_width` in wide mode). Force an **opaque background**; display `width:100%`. | DPI is derived from the known layout fact, not a magic `px_per_unit`. Verified: `main { max-width: calc(700px + 25px + 6px) }` is a hard column. Opaque bg avoids the dark-mode/transparent-bg footgun. Hit-testing stays correct via runtime `getBoundingClientRect` scale regardless. **This policy bounds the base64 PNG (display-bounded).** A sub-pixel `:grid` is bounded the same way: one source value per screen pixel, not the source matrix (`perf-findings.md`, [§8](architecture/08-scaling.md)). A grid whose cells are already at least one screen pixel still ships the whole source matrix. That matrix is no larger than the viewport only while the grid fills the axis; zooming until the average cell is at least one screen pixel ships every cell, including those outside the viewport. |
| **Wide mode** | **Package-owned.** The `max_width` keyword (default 700) exists and sets the render width (`2 × max_width`). The other half, vendoring `PlutoUI.WideCell`'s technique (cell `width` + negative `margin-left` + editor `ResizeObserver`) **inside our widget** so the cell itself widens, is **not built** (#179). | `PlutoUI.WideCell` **no-ops under `@bind`** (it checks `parentElement === PLUTO-OUTPUT`; the `<bond>` breaks that) and can't tell our renderer how big to draw (→ blurry on widen). Owning it sidesteps both and removes a PlutoUI dep. The rare case where reimplementing beats reuse. |
| **JS testing** | **vitest** for hit-test geometry **and** overlay visual recipes (wash / ring / hover outline / remount fade / `prefers-color-scheme` CSS); the **Pluto + Playwright** harness for browser integration as required CI (`bind_click.mjs` / `click.mjs`). The kind-sweep + polish-verify playbook (`live-interaction-checklist.md`) covers interaction **and** visual on every backend; CI's `kind-sweep` job runs those drivers (plus `keyboard_a11y.mjs`) on both backends, but only advisorily (`continue-on-error: true`), so agents still run the playbook locally. | Geometry and chrome tokens are unit-testable in Node; live hover/click/`@bind` and on-screen recipes need a real Pluto + browser. |
| **Dependencies** | **Pluto-facing Julia deps are hard deps:** `AbstractPlutoDingetjes` + `HypertextLiteral` (no package extension; the Makie backends and DataFrames are the weak deps with extensions). **JS runtime dep (bundled):** `d3-format` v3 (M2.3; first runtime JS dep; bundle-size delta in `perf-findings.md`). | For a Pluto-targeted widget the extension only buys "core usable headless," which isn't a real use case here — it'd be ceremony. JS deps are bundled into `assets/overlay.js` — no CDN, works offline. |
...
```
masque show()  emits (`:cairo`; src/render.jl):
  <div class="ip-host">
    ...
    <script>
      ...
      const requestFrame = $(with_js_link(...) or null) // gesture channel, view layers only
      window.Masque.mount(currentScript, manifest, invalidation, requestFrame)
      // mount() builds the shadow root, its styles, and the overlay svgs in the browser
    </script>
  </div>
```
