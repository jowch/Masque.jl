...
| **Manifest transport** | **`published_to_js`** (MsgPack on the wire), not inlined JSON. | Binary-efficient, no HTML bloat — **and verified to survive static export with no kernel** (below). Inlined JSON remains the trivial fallback for non-Pluto contexts. *(Not a "lossless Float64" win — geometry is quantized to `Int` pixels ([§9](architecture/09-wire-encoding.md#9-wire-encoding--precision)) and JSON would be lossless for coords too; the real point is binary compactness.)* Efficient for typical single plots (50–400 KB); but the manifest is the O(#hit-elements) / O(source-cells) scaling term and reaches multi-MB at high-N scatter / large grids — see `perf-findings.md` / [§8](architecture/08-scaling.md#8-payload-scaling--robustness-to-large-inputs). |
