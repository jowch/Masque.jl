...
(Element geometry ships as `Vector{Int}` — quantized to integer pixels (architecture/09-wire-encoding.md §9), so the
...
in PR #9** (element geometry vectors are `Int`, `architecture/09-wire-encoding.md §9`). The
...
- **Phase 2a bars/areas/spans** *(delivered, 2026-06-30)* — Hist, Waterfall, CrossBar, HSpan,
  ...
  shape per surface type (a few numbers per element — see `architecture/03-interactables.md` §3 bar payload
...
- **Phase 2b polygon surfaces** *(delivered, commit `431bd99`, 2026-06-30)* — Band, Density,
  ...
  1–3 B/coord path as scatter; see architecture/09-wire-encoding.md §9). Bench §F (2026-06-30, `bench/payload_envelope.jl`):
...
backend-symmetric Julia re-render, on the gesture channel rather than `@bind` — #122,
`architecture/12-gesture-channel.md` §12.3; only the client-side GPU camera is out of scope — see
