...
| Crosshair | overlay chrome on `svg.masque-plain` | none | both hairlines whenever the cursor is `crosshair` (axis, colorbar, grid, empty axis interior, a layer a slice covers); off on a discrete mark, legend, threshold, ROI, and view-grab; 80–120 ms fade on enter/leave of that state, not on each move |
| Slice | `:slice` (not a hit target) | none | covered lines: cross on, line highlight skipped, tooltip names each series at the probe (`wide 1.000` / `narrow 3.000` at the two probes); covered density: both labels in the tooltip; a discrete mark beside a slice keeps `pointer`, its own tooltip, and the cross off |
