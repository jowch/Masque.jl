...
| View (2D pan / 3D orbit) | `:view` | none | **commits nothing** (§12.3) — drag readout only, no `@bind` write. On `:cairo`: in-drag frames stream over the gesture channel (`with_js_link`, #102) — the base image and hit manifest swap together on every camera move, `px_per_unit` drops to 1 mid-gesture and restores on release, a pre-existing selection on another layer survives the swap. On `:webgl`: no live-preview mechanism yet (§12.10) — the readout still works, nothing repaints |
