...
begin
    ...
    fig = Figure(size = (620, 400), figure_padding = (4, 4, 4, 4))
    ax = Axis(fig[1, 1]; title = "New York City", aspect = DataAspect())
    hidedecorations!(ax)
    hidespines!(ax)
    p = poly!(ax, rings; color = pops, colormap = :deep, colorrange = (4.0e5, 2.7e6), strokecolor = :white, strokewidth = 1.5)
    Colorbar(
        fig[1, 2], p;
        label = "Population",
        ticks = [5.0e5, 1.5e6, 2.5e6],
        tickformat = xs -> ["500k", "1.5M", "2.5M"],
    )
